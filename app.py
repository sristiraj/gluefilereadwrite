...
for idx, path in enumerate(s3_paths):
    ...
    df1.write.format("parquet").mode("overwrite").option("header","true").option("path","s3://{}/{}/{}".format(s3_bucket, db_s3_path, tbl_name.replace("-","_"))).saveAsTable("`{}`.{}".format(glue_db_name, tbl_name.replace("-","_")))    
