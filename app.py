...
for idx, path in enumerate(s3_paths):
    ...
    df = spark.read.option("delimiter",sep).option("header","true").option("quote","\"").csv(read_path.format(s3_bucket, path.strip("/")), unescapedQuoteHandling="STOP_AT_DELIMITER")
    ...
    df1.write.format("parquet").mode("overwrite").option("header","true").option("path","s3://{}/{}/{}".format(s3_bucket, db_s3_path, tbl_name.replace("-","_"))).saveAsTable("data-lake-data-dictionary.{}".format( tbl_name.replace("-","_")))    
